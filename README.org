...
+ Easy serialisation :: Straightforward interoperability with JSON (via either
  ~JSON.jl~ or ~JSON3.jl~) and XML via ~@jsondef~ and ~@xmldef~.
+ Request payloads :: JSON bodies, plus urlencoded and multipart form
  submissions via ~@formdef~ and ~@multipartdef~.
