...
macro xpath(spec::String)
    body = :node
    terminated = false
    for component in eachsplit(spec, '/', keepempty=false)
        if terminated
        ...
        elseif '[' in component &&endswith(component, ']')
            ...
            body = :(extract_child($body, $(String(component))))
        end
    end
    Expr(:->, :node, body)
end
...
parsexml(::Type{T}, content::String) where {T} = parse(T, XML.unescape(content))
